...
object AdNetworkAnalysis {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 3. 列裁剪, 过滤, 去重
    source.selectExpr("networkmannerid", "networkmannername", "adplatformproviderid", "requestmode", "processnode",
      "iseffective", "isbilling", "isbid", "iswin", "adorderid", "adcreativeid", "winprice", "adpayment")
      .filter("networkmannerid is not null and networkmannername is not null and networkmannername != ''")
    ...
    // 5.2 指定主键字段
    val keys = Seq("networkmannerid", "networkmannername")
