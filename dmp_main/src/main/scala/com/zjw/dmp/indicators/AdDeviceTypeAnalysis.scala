...
object AdDeviceTypeAnalysis {
  ...
  // 定义数据存入表
  val SINK_TABLE = s"ad_device_analysis_${DateUtils.getNow()}"
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 3. 列裁剪, 过滤, 去重
    source.selectExpr("client", "device", "adplatformproviderid", "requestmode", "processnode",
      "iseffective", "isbilling", "isbid", "iswin", "adorderid", "adcreativeid", "winprice", "adpayment")
      .filter("client is not null and device is not null and device != ''")
